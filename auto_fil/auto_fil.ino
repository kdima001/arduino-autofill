...
//помпа осмоса (повышения давления) - подача RO
#define PUMP_RO			3
//клапана
#define V_RAW			2 //Главный клапан (на входе из водопровода) и фильтрованная вода
#define V_DRAIN2		5 //Слив второй емкости-дозатора
#define V_FILL1			9 //Наполнение первого дозатора
#define V_DRAIN1		10 //Слив первого дозатора

#define V_FLUSH			4 //Клапан промывки
#define PR_SENS			A7//Датчик напряжения после двигателя - контроль датчика минимального давления
											//при замкнутом датчике (есть давление) на A7 напряжение ~2.4V, если разомкнут датчик
											//высокого давления напряжение будет ~1.6V. Напряжение будет =0 если разомкнуты оба датчика 
											//или датчик низкого давления. т.е:
											//~0 		авария, нет давления воды
											//~1.6	давление превышает норму (подумать в сторону контроля забития мембраны)
											//~2.4	норма
											
#define V_TOPUP			A0 //Управление доливкой в канал 1 (первый дозатор)

//прочее
#define MIXER				6 //мешалка

//константы интервалов в мсек
#define POOL_INT		1000
//Время перемешивания
#define T_MIXING		120000
//Время до включения миксера при наполнении
#define T_MIXING_FILL 3600000
//Время перемешивания после наполнения (8 часов)
#define T_MIXING_AF		28800000 
//Время наполнения дозатора 1
#define T_FILL1			300000
//Время слива (после срабатывания нижнего датчика) дозатора 1 (малого)
#define T_DRAIN1		300000
//Время слива дозатора 2 (большого)
#define T_DRAIN2		30000
//Время на слив второго дозатора (если датчик глючит)
#define T_DRAIN_TIMEOUT 900000

//Время на наполнение второго дозатора (до аварии)
#define T_FILL2			3600000

//Время паузы внутренними циклами первого дозатора СЕКУНДЫ
#define T_WAIT1			1800
//Время паузы внутренними циклами второго дозатора СЕКУНДЫ
#define T_WAIT2			300
...
//------------------------------------------------------------
// Часы РВ (RTC)
#define CE_PIN	 6		//то же что и на mixer
DS1302RTC RTC(CE_PIN, A4, A5);
//Данные для работы с часами
tmElements_t tm; 
time_t t; 
//---------------------------------------------------------------------------
//задатчик кол-ва подмен
int8_t TargetCount1, TargetCount2, RO_part;
//Переменные состояния
uint8_t CurrentCount1, CurrentCount2;
...
enum mRO_t {						//режим работы осмосного накопителя
 ...
 RO_OVER,							//Наполнение сверх датчика
...
enum CON_t {			//режим работы емкости с концентратом
	C_UNDEF,
	C_READY,			//Емкость готова
	C_EMPTY,			//Емкость пуста. стоп
	C_DRAIN,			//Слив до датчика
	C_DRAIN_T,			//Слив до времени
	C_FILL,				//Емкость наполняется, в адрес 0 пишется статус
	C_FILL_WAIT_MIXING,	//Емкость наполняется, ждем время до включения миксера (защита от сухого хода и высоких концентраций порошка)
	C_FILL_MIXING,		//Емкость наполняется, включен миксер, в адрес 0 пишется статус
	C_MIXING,			//Емкость перемешивается, включен миксер, в адрес 0 пишется статус
	C_DISABLE
};
CON_t CON;

void PrintStatusCON(void) {	
	PrintTime();
	Serial << F(" CON = ");
	switch(CON) {
		case C_UNDEF: 				Serial << F("C_UNDEF") << endl; break;
		case C_READY: 				Serial << F("C_READY") << endl; break;
		case C_EMPTY: 				Serial << F("C_EMPTY") << endl; break;
		case C_DRAIN: 				Serial << F("C_DRAIN") << endl; break;
		case C_DRAIN_T: 			Serial << F("C_DRAIN_T") << endl; break;
		case C_FILL: 				Serial << F("C_FILL") << endl; break;
		case C_FILL_WAIT_MIXING: 	Serial << F("C_FILL_WAIT_MIXING") << endl; break;
		case C_FILL_MIXING: 		Serial << F("C_FILL_MIXING") << endl; break;
		case C_MIXING: 			    Serial << F("C_MIXING") << endl; break;
		case C_DISABLE: 			Serial << F("C_DISABLE") << endl; break;
	};
} 

enum CH1_t {
	C1_UNDEF,
	C1_READY,						//Емкость готова, в адрес 0 пишется статус
	C1_MIXING,						//Помешаем концентрат
	C1_FILL,						//Емкость готова, дозатор наполняется
	C1_DRAIN,						//Емкость готова, дозатор сливается
	C1_WAIT,						//Итерация цикла завершена, ждем заданную паузу
	C1_STOP,						//Цикл закончен
  C1_DISABLE						//Работа запрещена
};
CH1_t CH1;

void PrintStatusCH1(void) {
	PrintTime();
	Serial << F(" CH1 = ");
	switch(CH1) {
		case C1_UNDEF: 		Serial << F("C1_UNDEF") << endl; break;
		case C1_READY: 		Serial << F("C1_READY") << endl; break;
		case C1_MIXING: 	Serial << F("C1_MIXING") << endl; break;
		case C1_FILL: 		Serial << F("C1_FILL") << endl; break;
		case C1_DRAIN: 		Serial << F("C1_DRAIN") << endl; break;
		case C1_WAIT: 		Serial << F("C1_WAIT") << endl; break;
		case C1_STOP: 		Serial << F("C1_STOP") << endl; break;
		case C1_DISABLE: 	Serial << F("C1_DISABLE") << endl; break;
	};
} 

enum CH2_t {		  	// Режим работы 2й емкости
	C2_UNDEF,
	C2_READY,			//Емкость пуста
	C2_EMPTY,			//Емкость пуста
	C2_FILL_RAW, 		//Емкость наполняется фильтрованной водой
	C2_FILL_RO,			//Емкость наполняется осмосом
	C2_DRAIN,			//Емкость сливается
	C2_DRAIN_T,			//Емкость сливает остатки
	C2_WAIT,			//Ждем 
	C2_STOP,			//Цикл закончен
	C2_DISABLE 	 	  	//Работа запрещена
};
CH2_t CH2;

void PrintStatusCH2(void) {	
	PrintTime();
  Serial << F(" CH2 = ");
	switch(CH2) {
		case C2_UNDEF: 		Serial << F("C2_UNDEF") << endl; break;
		case C2_READY: 		Serial << F("C2_READY") << endl; break;
		case C2_EMPTY: 		Serial << F("C2_EMPTY") << endl; break;
		case C2_FILL_RAW:	Serial << F("C2_FILL_RAW") << endl; break;
		case C2_FILL_RO: 	Serial << F("C2_FILL_RO") << endl; break;
		case C2_DRAIN: 		Serial << F("C2_DRAIN") << endl; break;
		case C2_DRAIN_T: 	Serial << F("C2_DRAIN_T") << endl; break;
		case C2_WAIT: 		Serial << F("C2_WAIT") << endl; break;
		case C2_STOP: 		Serial << F("C2_STOP") << endl; break;
		case C2_DISABLE: 	Serial << F("C2_DISABLE") << endl; break;
	};
}
//---------------------------------------------------------------------------
void(* Reset) (void) = 0; // Reset CON function
//---------------------------------------------------------------------------
uint16_t GetCrossMatrixValue(uint8_t v, uint8_t g) { 
	uint16_t val;
	//Столбец (вертикаль) на вывод
	pinMode(v, OUTPUT);
	//Столбец в 1
	digitalWrite(v, HIGH);
	//Подождем пока зарядятся емкости.
	delay(200); wdt_reset();
	//Строка (горизонталь) на ввод
	pinMode(g, INPUT);
	//Читаем строку
	//Пустое чтение для исключение помех (по опыту хватит 2х, но 5 для надежности)
	analogRead(g); analogRead(g); analogRead(g); analogRead(g); analogRead(g);
	delay(100); wdt_reset();
	val = analogRead(g);
	/*delay(10); wdt_reset();
	val = val + analogRead(g);
	delay(10); wdt_reset();
	val = val + analogRead(g);
	delay(10); wdt_reset();
	val = val + analogRead(g);
	...
	val = val + analogRead(g);
	val = val / 5;*/
	//Столбец прижимаем к земле
	digitalWrite(v, LOW);
	//Строку на ввод (пассивный, чтобы не мешал)
	pinMode(g, INPUT);
	return val;
}
//---------------------------------------------------------------------------
...
void SetCNT(void) {
/*
Задатчик кол-ва подмен 1 (малый)		7-A3
Задатчик кол-ва подмен 2	(большой)	7-A2
Задатчик концентрации RO						7-A1
Задатчик "резерв"										7-A7
Уровень в емк 1											8-A3
Уровень в емк 2											8-A2
Верхний датчик RO										8-A1
Датчик уровня в аквариумах					8-A7*/
/* v_max, v_min, n */
	TargetCount1 = constrain(round(map_float(GetCrossMatrixValue(7, A3), 60, 889, 0, 20)), 0, 20);
	TargetCount2 = constrain(round(map_float(GetCrossMatrixValue(7, A2), 90, 905, 0, 20)), 0, 20);
	RO_part		 = constrain(round(map_float(GetCrossMatrixValue(7, A1), 50, 900, 0, 10))*10, 0, 100);
}
//---------------------------------------------------------------------------
void PrintTime(void){ 
